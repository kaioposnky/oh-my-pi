...
- Updated Google Antigravity image generation to use `gemini-3.1-flash-image` (Nano Banana 2) by default, fixing text-fallback responses from Cloud Code Assist for `gemini-3-pro-image`.
- Added `providers.imageAntigravityModel` setting and `PI_ANTIGRAVITY_IMAGE_MODEL`/`ANTIGRAVITY_IMAGE_MODEL` environment variable support to configure the Antigravity image model.
